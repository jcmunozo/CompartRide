...
from cride.circles import seri
...
class RideModelSerailizer(serializers.ModelSerializer):
    """Ride model serailizer."""
    ...
    def update(self, instance, data):
        ...
        return super(RideModelSerailizer, self).update(instance,data)

class CreateRideSerializer(serializers.ModelSerializer):
    """Create ride serializer."""

    offered_by = rerializers.HiddenField(default=serializers.CurrentUserDefault())
    available_seats = serailizers.IntegerField(min_value=1, max_value=15)
    ...
    def validate_departure_date(self, data):
        ...
        if data < min_date:
            raise serializars.ValidationError(
                    'Departure time must be at least pass the next 20 minutes window.'
                    )
        return data
    def validate(self, data):
        """Validate.
        Verify that the person who offers thr ride is member
        and also the same user making request
        """
    ...
    class JoinRideSerializer(serializers.ModelSerializer):
        ...
        def validate(self, data):
            ...
            if Ride.objects.filter(passengers__pk=data['passenger']):
                raise serializers.ValidationError('Passenger is already in trip.')
    ...
    class EndRideSerializer(serializers.ModelSerializer):
        ...
        def validate_current_time(self, data):
            ...
            if data <= ride.departure_date:
                raise serializers.ValidationError('Ride has no started yet')
            return data
